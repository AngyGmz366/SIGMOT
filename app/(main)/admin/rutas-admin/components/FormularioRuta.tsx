...
import { validarTiempoEstimado, validarHorario } from '@/utils/timeValidation';
import TimePickerHMS from '@/components/admin/rutas/TimePickerHMS';
import TimePickerHM from '@/components/admin/rutas/TimePickerHM';
...
const FormularioRuta: React.FC<FormularioRutaProps> = ({ ruta, onGuardar, onCerrar, loading, unidadesDisponibles }) => {
    // ---------- Estado base ----------
    ...
    const [errors, setErrors] = useState<{ tiempoEstimado?: string; horario?: Record<number, string | undefined> }>({});
    ...
    // ---------- Helpers ----------
    const getLocalidad = (nombre: string) => localidades.find((l) => l.nombre.toLowerCase() === (nombre || '').toLowerCase());
    ...
    const handleText = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        if (name === 'tiempoEstimado') {
            const error = validarTiempoEstimado(value);
            setErrors((prev) => ({ ...prev, tiempoEstimado: error || undefined }));
        }
    };
    ...
    const asignacionesCompletas = asignaciones.every((a, i) => a.unidadId !== 0 && a.horario !== '' && !errors.horario?.[i]);

    // ---------- Submit ----------
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();

        const errorTiempo = validarTiempoEstimado(formData.tiempoEstimado || '');
        if (errorTiempo) {
            setErrors((prev) => ({ ...prev, tiempoEstimado: errorTiempo }));
            alert('⚠️ ' + errorTiempo);
            return;
        }
        ...
        <Card title={ruta && ruta.id !== 0 ? 'Editar Ruta' : 'Nueva Ruta'} className="shadow-2 border-round-xl">
            <form onSubmit={handleSubmit} className="p-fluid grid formgrid">
                ...
                <div className="field col-12 md:col-6">
                    ...
                    <Dropdown value={formData.origen || null} options={opcionesLocalidades} onChange={handleOrigen} placeholder="Seleccione una ciudad" filter showClear className="w-full" />
                </div>

                <div className="field col-12 md:col-6">
                    <label className="font-medium">Destino</label>
                    <Dropdown value={formData.destino || null} options={opcionesLocalidades} onChange={handleDestino} placeholder="Seleccione destino" filter showClear className="w-full" />
                </div>
                ...
                <div className="field col-12 md:col-6">
                    <TimePickerHMS
                        value={formData.tiempoEstimado || '00:00'}
                        onChange={(val) => {
                            setFormData((prev) => ({ ...prev, tiempoEstimado: val }));
                            const error = validarTiempoEstimado(val);
                            setErrors((prev) => ({ ...prev, tiempoEstimado: error || undefined }));
                        }}
                ...
                <div className="field col-12 md:col-6">
                    ...
                    <InputText id="precio" name="precio" value={formData.precio?.toString() ?? ''} onChange={handleText} placeholder="250.00" />
                </div>

                <div className="field col-12">
                    <label className="font-medium">Descripción</label>
                    <InputTextarea id="descripcion" name="descripcion" rows={2} value={formData.descripcion ?? ''} onChange={handleText} placeholder="Descripción de la ruta" />
                </div>
                ...
                <div className="field col-12">
                    ...
                    {asignaciones.map((a, index) => (
                        <div key={index} className="grid mt-2 p-3 border-1 border-round" style={{ backgroundColor: '#f8f9fa' }}>
                            ...
                            <div className="col-12 md:col-5">
                                <TimePickerHM value={a.horario} onChange={(val) => actualizarAsignacionHorario(index, val)} error={errors.horario?.[index]} label="Horario (HH:mm)" />
                            </div>

                            <div className="col-12 md:col-1 flex align-items-center justify-content-center">{a.unidadId !== 0 && a.horario && /^\d{2}:\d{2}$/.test(a.horario) && <i className="pi pi-check text-green-500"></i>}</div>
                        </div>
                    ))}
                    ...
                    {asignaciones.some((x) => x.unidadId !== 0) && (
                        <div className="mt-3">
                            ...
                            <DataTable value={asignaciones.filter((x) => x.unidadId !== 0)} className="p-datatable-sm" size="small">
                                ...
                                <Column header="Estado" body={(d: UnidadAsignada) => (d.unidadId !== 0 && d.horario && /^\d{2}:\d{2}$/.test(d.horario) ? <Tag value="Completo" severity="success" /> : <Tag value="Incompleto" severity="warning" />)} />
                            </DataTable>
                        </div>
                    )}
                </div>
                ...
                <div className="field col-12">
                    ...
                    <InputTextarea
                        ...
                        name="coordenadas"
                        rows={3}
                        value={Array.isArray(formData.coordenadas) && formData.coordenadas.length > 0 ? JSON.stringify(formData.coordenadas, null, 2) : 'Selecciona origen y destino para generar coordenadas'}
                        onChange={handleText}
                    />
                </div>
                ...
                <div className="col-12 flex justify-end gap-2 mt-3">
                    ...
                    <Button type="submit" label="Guardar" icon="pi pi-check" className="p-button-primary p-button-sm" loading={loading} disabled={!asignacionesCompletas || !!errors.tiempoEstimado} />
                </div>
            </form>
        </Card>
    );
